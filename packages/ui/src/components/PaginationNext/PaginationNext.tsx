...
export type PaginationNextVariant = 'primary' | 'secondary';
export type PaginationNextSize = 'medium';

export interface PaginationNextProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: PaginationNextVariant;
  size?: PaginationNextSize;
  fullWidth?: boolean;
  leftIcon?: React.ReactNode;
  rightIcon?: React.ReactNode;
}

const sizeStyles = (t: DefaultTheme): Record<PaginationNextSize, ReturnType<typeof css>> => ({
  medium: css`height: 40px; padding: 0 ${t.spacing[4]}; font-size: ${t.typography.fontSize.md}; gap: ${t.spacing[2]};`,
});

const variantStyles = (t: DefaultTheme): Record<PaginationNextVariant, ReturnType<typeof css>> => ({
  primary: css`
...
const Root = styled.button<{ $variant: PaginationNextVariant; $size: PaginationNextSize; $fullWidth: boolean }>`
  display: inline-flex;
  align-items: center;
